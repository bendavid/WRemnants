...
parser.add_argument(
    "--histmaker",
    type=str,
    help="histmaker to perform bias tests with",
    default="mw_with_mu_eta_pt",
    choices=["mw_with_mu_eta_pt", "mz_wlike_with_mu_eta_pt", "mz_dilepton"],
)
parser.add_argument(
    "-o",
    "--outfolder",
    type=str,
    default="./hdf5Files/nonClosureCorl",
    help="Output folder",
)
parser.add_argument(
    "--combineOutFolder",
    type=str,
    default="./CombineStudies/nonClosureCorlSigOnly/",
...
parser.add_argument(
    ...
    type=int,
    default=192,
    help="number of threads to run the histmaker",
)
parser.add_argument(
    "--infoOnly",
    action="store_true",
...
datasets = [
    ...
    (
        "--smearing --nonClosureScheme A-M-separated  --correlatedNonClosureNP",
        "--smearing --biasCalibration parameterized",
    ),
    (
        "--smearing --nonClosureScheme binned --correlatedNonClosureNP",
        "--smearing --biasCalibration binned",
    ),
...
freeze_uncertainties = [
    (
        "reducedUncertainties",
        "-x '.*' -k '(mass.*|muonScaleSyst.*|CMS_scale_m.*|Z_non_closure.*|Z_nonClosure.*)'",
    ),  # freeze all nuisances except momentum scale #-k '(CMS_scale_m.*|mass.*|Z_nonClosure.*)'
    ("fullUncertainties", ""),
]
...
for nominal, pseudodata in datasets:
    ...
    file_nominal = (
        f"{args.outfolder}/{histmaker}_{args.defaultPostfix}_{str_nominal}.hdf5"
    )
    file_sep_impact = (
        file_nominal if not "BinnedPlus" in file_nominal else file_sep_impact
    )
    file_pseudodata = (
        f"{args.outfolder}/{histmaker}_{args.defaultPostfix}_{str_pseudodata}.hdf5"
    )
    ...
    for freeze_name, freeze_command in freeze_uncertainties:
        ...
        dir_combine = (
            f"{args.combineOutFolder}/{str_nominal}_vs_{str_pseudodata}_{freeze_name}"
        )
        ...
        file_nominal_comb = file_nominal.replace(
