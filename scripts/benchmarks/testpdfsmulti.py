...
using tensor_t = Eigen::TensorFixedSize<double, Eigen::Sizes<103, 10>>;

tensor_t make_tensor(const ROOT::VecOps::RVec<float> &vec, double weight) {
  tensor_t res;

  for (int i = 0; i < 103; ++i) {
    res(i, 0) = weight*vec[i];
  }
...
def build_graph(df, dataset):
    ...
    if args.useTensor:
        ...
        df = df.Define("pdfWeights_tensor", "wtest::make_tensor(LHEPdfWeight, weight)")
