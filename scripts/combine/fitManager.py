...
def prepareChargeFit(options, charges=["plus"]):
    ...
    binname = (
        "ZMassWLike"
        if options.isWlike
        else "ZMassDilepton" if options.isDilepton else "WMass"
    )

    for charge in charges:
        datacards.append(
            os.path.abspath(options.inputdir)
            + "/{b}_{ch}.txt".format(b=binname, ch=charge)
        )
    ...
    if options.theoryAgnostic:
        datacards.append(
            os.path.abspath(options.inputdir)
            + "/{b}_inclusive_xnorm.txt".format(b=binname)
        )
    ...
    if sum([os.path.exists(card) for card in datacards]) == len(datacards):
        ...
        combinedCard = "{d}/{b}_{s}.txt".format(
            d=os.path.abspath(cardSubfolderFullName), b=binname, s=cardkeyname
        )
        ccCmd = "combineCards.py --noDirPrefix {cards} > {combinedCard} ".format(
            cards=" ".join(
                [
                    "{ch}={dcfile}".format(ch=channels[i], dcfile=card)
        ...
        txt2hdf5Cmd = "text2hdf5.py {cf} --dataset {dn}".format(
            cf=combinedCard, dn=options.dataname
        )
        if options.theoryAgnostic:
            maskchan = [
                "--maskedChan {mc}".format(mc=maskedChannel)
                for maskedChannel in maskedChannels
            ]
            txt2hdf5Cmd += " --sparse {mc} --X-allow-no-background".format(
                mc=" ".join(maskchan)
            )
        else:
        ...
        if options.clipSystVariations > 0.0:
            txt2hdf5Cmd = (
                txt2hdf5Cmd + " --clipSystVariations " + str(options.clipSystVariations)
            )
        if options.clipSystVariationsSignal > 0.0:
            txt2hdf5Cmd = (
                txt2hdf5Cmd
                + " --clipSystVariationsSignal "
                + str(options.clipSystVariationsSignal)
            )
        ...
        globImpTag = ""
        if not options.noCorrelateXsecStat:
            bbboptions += "--correlateXsecStat "
        combineCmd = "combinetf.py -t -1 {bbb} {metafile} --doImpacts --saveHists --computeHistErrors ".format(
            metafile=metafilename, bbb="" if options.noBBB else bbboptions
        )
        ...
        print(
            "Use the following command to run combine (add --seed <seed> to specify the seed, if needed). See other options in combinetf.py"
        )
        print
        ...
        bbbtext = (
            "0"
            if options.noBBB
            else "1_cxs0" if options.noCorrelateXsecStat else "1_cxs1"
        )
        combineCmd_data = (
            combineCmd_data
            + " --postfix Data{pf}_bbb{b}{git} --outputDir {od} ".format(
                pf=fitPostfix, od=fitdir_data, b=bbbtext, git=globImpTag
            )
        )
        combineCmd_Asimov = (
            combineCmd
            + " --postfix Asimov{pf}_bbb{b}{git} --outputDir {od} ".format(
                pf=fitPostfix, od=fitdir_Asimov, b=bbbtext, git=globImpTag
            )
        )
        combineCmd_toys = (
            combineCmd_toys
            + " --postfix Toys{pf}_bbb{b}{git} --outputDir {od} ".format(
                pf=fitPostfix, od=fitdir_toys, b=bbbtext, git=globImpTag
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="input directory with the root files inside (cards are eventually stored in the foldr specified with option --cf)",
    )
    parser.add_argument(
        "--cf",
        "--card-folder",
        ...
        default="nominal",
        type=str,
        help="Subfolder created inside inputdir to store all cards and fit results in a given configuration, for better bookkeeping when doing tests",
    )
    parser.add_argument(
        "-c",
        "--charge",
        ...
        type=str,
        help="process given charge. default is both",
    )
    parser.add_argument(
        "--comb",
        dest="combineCharges",
        default=False,
        action="store_true",
        help="Combine W+ and W-, if single cards are done",
    )
    parser.add_argument(
        "--fit-single-charge",
        dest="fitSingleCharge",
        default=False,
        action="store_true",
        help="Prepare datacard for single-charge fit. For each charge, a postfix is appended to option --postfix, so no need to add the charge explicitly",
    )
    parser.add_argument(
        "--postfix",
        dest="postfix",
        type=str,
        default="",
        help="Postfix for .hdf5 file created with text2hdf5.py",
    )
    parser.add_argument(
        "--wlike",
        dest="isWlike",
        action="store_true",
        default=False,
        help="Make cards for the Z W-like analysis. Default is Wmass",
    )
    parser.add_argument(
        "--dilepton",
        dest="isDilepton",
        action="store_true",
        default=False,
        help="Make cards for the Z dilepton analysis. Default is Wmass",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=-1.0,
        help="Clipping of syst variations, passed to text2hdf5.py",
    )
    parser.add_argument(
        "--clipSystVariationsSignal",
        type=float,
        default=-1.0,
        help="Clipping of signal syst variations, passed to text2hdf5.py",
    )
    parser.add_argument(
        "--no-bbb",
        dest="noBBB",
        default=False,
        action="store_true",
        help="Do not use bin-by-bin uncertainties",
    )
    parser.add_argument(
        "--correlate-xsec-stat",
        dest="noCorrelateXsecStat",
        default=True,
        action="store_false",
        help="If given, use option --correlateXsecStat when using bin-by-bin uncertainties (for mass measurements it should not be needed because we do not use prefit cross sections)",
    )
    parser.add_argument(
        "-d",
        "--dry-run",
        ...
        default=False,
        action="store_true",
        help="Do not execute command to make cards or fit",
    )
    parser.add_argument(
        "--doOnlyCard",
        dest="doOnlyCard",
        default=False,
        action="store_true",
        help="Do only card and exit (equivalent to using --no-text2hdf5 and --no-combinetf together)",
    )
    parser.add_argument(
        "--no-card",
        dest="skip_card",
        default=False,
        action="store_true",
        help="Go directly to fit part without regenerating the card (can also skip text2hdf5 with --no-text2hdf5), useful when editing the card manually for tests",
    )
    parser.add_argument(
        "--no-text2hdf5",
        dest="skip_text2hdf5",
        default=False,
        action="store_true",
        help="skip running text2hdf5.py at the end, only prints command (useful if hdf5 file already exists, or for tests)",
    )
    parser.add_argument(
        "--no-combinetf",
        dest="skip_combinetf",
        default=False,
        action="store_true",
        help="skip running combinetf.py at the end, just print command (useful for tests)",
    )
    parser.add_argument(
        "--skip-fit-data",
        dest="skipFitData",
    ...
    parser.add_argument(
        ...
        default="data_obs",
        type=str,
        help="Name of the observed dataset (pass name without x_ in the beginning). Useful to fit another pseudodata histogram",
    )
    parser.add_argument(
        "--combinetf-option",
        dest="combinetfOption",
        default="",
        type=str,
        help="Pass other options to combinetf (TODO: some are already activated with other options, might move them here)",
    )
    parser.add_argument(
        "-t",
        "--toys",
        type=int,
        default=0,
        help="Run combinetf for N toys if argument N is positive",
    )
    parser.add_argument(
        "--theoryAgnostic",
        action="store_true",
        help="Run theory agnostic fit, with masked channels and so on (not needed when using POIs as NOIs)",
    )
    parser.add_argument(
        "--globalImpacts", action="store_true", help="Use global impacts for combinetf"
    )
    ...
    if not args.dryRun:
        ...
        if cmssw == "":
            ...
            print(
                "You should work from a cmssw-cc7 singularity environment to get the release."
            )
            print("Aborting ...")
            print("\n")
            quit()
    ...
    if "ZMassWLike_" in args.inputdir and not args.isWlike:
        print(
            "Warning: ZMassWLike found in input path {}, but option --wlike not specified, please check".format(
                args.inputdir
            )
        )
        quit()
    if "ZMassDilepton_" in args.inputdir and not args.isDilepton:
        print(
            "Warning: ZMassDilepton found in input path {}, but option --dilepton not specified, please check".format(
                args.inputdir
            )
        )
        quit()
    ...
    if args.cardFolder:
        ...
        createFolder(
            cardFolderFullName, dryRun=False
        )  # always create this folder, even for tests
    ...
    if not args.combineCharges and not args.fitSingleCharge:
        print(
            "Warning: must pass one option between --fit-single-charge and --comb to fit single charge or combination."
        )

    if args.combineCharges:
        if args.fitSingleCharge:
            print(
                "Error: options --fit-single-charge and --comb are incompatible. Abort"
            )
            quit()
        if len(fitCharges) != 2:
            print(
                "Error: --comb requires two charges, use -C 'plus,minus' and try again"
            )
            quit()
