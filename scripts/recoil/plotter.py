...
def translate(word):
    if "METPT" in word:
        return word.replace("METPT", "p_{T}^{miss} (GeV)")
    if "METWLIKEPT" in word:
        return word.replace("METWLIKEPT", "p_{T}^{miss} (W-like) (GeV)")
    if "METPHI" in word:
        return word.replace("METPHI", "#phi^{miss} (GeV)")
    if "METX" in word:
        return word.replace("METX", "p_{T,x}^{miss} (GeV)")
    if "METY" in word:
        return word.replace("METY", "p_{T,y}^{miss} (GeV)")
    if "UPARAQT" in word:
        return word.replace("UPARAQT", "U_{#parallel} #plus q_{T} (GeV)")
    if "UPARA" in word:
        return word.replace("UPARA", "U_{#parallel} (GeV)")
    if "UPERP" in word:
        return word.replace("UPERP", "U_{#perp}   (GeV)")
    if "UMAGN" in word:
        return word.replace("UMAGN", "|U| (GeV)")
    if "MT" in word:
        return word.replace("MT", "m_{T} (GeV)")
    if "PT" in word:
        return word.replace("PT", "p_{T} (GeV)")
    if "QT" in word:
        return word.replace("QT", "q_{T} (GeV)")
    if "HT" in word:
        return word.replace("HT", "H_{T} (GeV)")
    if "ETA" in word:
        return word.replace("ETA", "#eta")
    return word
...
def aux(c=None):
    ...
    if "_" in cfg["topRight"]:
        latex.DrawLatex(tr, 0.945, cfg["topRight"])
    elif "^" in cfg["topRight"]:
        latex.DrawLatex(tr, 0.945, cfg["topRight"])
    else:
        latex.DrawLatex(tr, 0.945, cfg["topRight"])  # was 955 ??  945
...
def dummyRatio(nbins=1, line=1):
    ...
    for i in range(0, nbins + 1):
        dummyT.SetBinContent(i, -1e9)
    for i in range(0, nbins + 1):
        dummyB.SetBinContent(i, -1e9)
    ...
    dummyB.GetXaxis().SetTitleOffset(
        1.0 * dummyB.GetXaxis().GetTitleOffset()
    )  ### was 4.0???
...
def canvasRatio():
    ...
    pad1 = ROOT.TPad(
        "p1%d" % random.randint(1e4, 1e5), "", 0, cfg["ratiofraction"], 1, 1
    )
    pad2 = ROOT.TPad(
        "p2%d" % random.randint(1e4, 1e5),
        "",
        0,
        0.0,
        1,
        cfg["ratiofraction"] - 0.7 * epsilon,
    )

    pad1.SetBottomMargin(epsilon)
    pad1.SetTopMargin(0.055 / (1.0 - cfg["ratiofraction"]))
