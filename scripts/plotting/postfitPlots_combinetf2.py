...
fitmode = "prefit" if args.prefit else "postfit"
...
hists_stack = combinetf2_input.load_histograms(fitresult, f"hist_{fitmode}", axes)
...
def make_plot(h_data, h_stack, axis, suffix):

    if ratio:
        fig, ax1, ax2 = plot_tools.figureWithRatio(h_data, styles.xlabels.get(axis, "Bin number"), "Entries/bin", args.ylim, "Data/Pred.", args.rrange)
    else:
        fig, ax1 = plot_tools.figure(h_data, styles.xlabels.get(axis, "Bin number"), "Entries/bin", args.ylim)
    ...
    if ratio:
        ...
        hep.histplot(
            hh.divideHists(sum(h_stack), sum(h_stack), cutoff=1e-8, rel_unc=True, flow=False, by_ax_name=False),
            histtype="step",
            color="grey",
            alpha=0.5,
            yerr=True,
        ...
        if data:
            hep.histplot(
                hh.divideHists(h_data, sum(h_stack), cutoff=0.01, rel_unc=False),
                histtype="errorbar",
                color="black",
                label="Data",
                yerr=False,
    ...
    to_join = [fitmode, args.postfix, axis, suffix]
...
for i, (hist_data, hist_stack) in enumerate(zip(hists_data_obs, hists_stack)):
    ...
    for axis in [n for n in axes_names if n not in args.selectionAxes]:
        ...
        make_plot(h_data, h_stack, axis, suffix=f"{suffix}")
        ...
        for sa in [a for a in axes[i] if a.name in args.selectionAxes]:
            for idx in range(len(sa)):
                ...
                make_plot(h_data, h_stack, axis, suffix=f"{suffix}{sel}")
