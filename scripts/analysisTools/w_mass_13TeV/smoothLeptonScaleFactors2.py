...
# python w_mass_13TeV/smoothLeptonScaleFactors2.py /path/to/tnp/efficiencies_ERA/mu_STEP_CHARGE/allEfficiencies_2D.root /path/to/tnp/smoothLeptonScaleFactors2/ --run-all [--do-merge] [--do-steps isonotrig iso isoplus isominus triggerplus triggerminus idip idipplus idipminus tracking trackingplus trackingminus reco recoplus recominus]

#
# TIPS:
# --> input path is usually the main folder produced by the TnP code, which contains a folder like efficiencies_GtoH/
# --> for bookkeeping it is better to create output folder smoothLeptonScaleFactors2/ where efficiencies_GtoH/ is located
...
def runFiles(args):
    ...
    for step in steps:
        ...
        cmd = f"python w_mass_13TeV/smoothLeptonScaleFactors2.py {inputFile} {args.outdir[0]} -c {charge} -s {step}"
