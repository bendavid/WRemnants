...
def niceName(name):
    ...
    if "helicity" in name:
        ...
        if "FakesEtaCharge" in name:
            ...
            tmpvar = "p_{T}-shape"
        return "QCD bkg {var}-{n} {lepCh}".format(
        ...
        if len(chargenum):
            ...
            chargeText = (
                "-" if "genQ0" in chargenum else "+" if "genQ1" in chargenum else ""
            )  # in case Z has a different convention
        if len(coeffnum):
...
def niceNameHEPDATA(name):
    ...
    if re.match("Fakes(Eta|EtaCharge|PtNorm|PtSlope)Uncorrelated.*", name):
        ...
        vlow = FakesBins[n - 1]
        vhigh = FakesBins[n]
        return "QCD bkg {var}, ${l}<{v}<{h}$, ${lepCh}$".format(
        ...
        num = re.findall(r"\d+", name)  # get number
        n1 = num[0]
        pfx = name.split("effStatTnP" + str(n1))[
            1
        ]  # split on second number and read what's on the right
        leptonCharge = ""
        if len(pfx):
            leptonCharge = "{lep}^{chs}".format(
                lep=r"\mu",  # if "mu" in pfx else "e",
        ...
        if "scaleStat" in name:
            ...
            return r"$p_{{T}}^{{{lep}}}$ scale syst.{n}, ${l}<\eta<{h}${ch}".format(
                ...
                ch=(
                    ", charge +"
                    if "plus" in name
                    else ", charge -" if "minus" in name else ""
                ),
            )
        ...
        n = int(num[0])
        etabinsEffSyst = (
            [0, 1.0, 1.5, 2.4] if "mu" in name else [0, 1.0, 1.5, 1.9, 2.1, 2.4]
        )
        ...
        if any(x == name for x in ["muR", "muF", "muRmuF"]):
            ...
            if n == 10:
                return "{s} {w}, $p_{{T}}^{{W}}>{l}$ {t}".format(
                    w=wch, s=scale, l=low, t=txt
                )
            else:
                return "{s} {w}, ${l}<p_{{T}}^{{W}}<{h}$ {t}".format(
                    w=wch, s=scale, l=low, h=high, t=txt
                )
...
if __name__ == "__main__":
    ...
    filter_matrixType_poiPostfix = {
        "channelpmaskedexp": "pmaskedexp",
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="output directory to save the matrix",
    )
    parser.add_argument(
        "-p",
        "--params",
        default="",
        type=str,
        help="parameters for which you want to show the correlation matrix. comma separated list of regexps",
    )
    parser.add_argument(
        "-t",
        "--type",
        default="hessian",
        choices=["toys", "scans", "hessian"],
        type=str,
        help="which type of input file: toys(default),scans, or hessian",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="Keyword for canvas name to uniquely identify the output plot",
    )
    parser.add_argument(
        "--vertical-labels-X",
    ...
    parser.add_argument(
        "--noTextMatrix",
        action="store_true",
        help="Do not print text with values on the matrix",
    )
    parser.add_argument("--title", default="", type=str, help="Title for matrix ")
    parser.add_argument(
        "--show-more-correlated",
        dest="showMoreCorrelated",
        default=0,
        type=int,
        help="Show the N nuisances more correlated (in absolute value) with the parameters given with --params. If 0, do not do this part",
    )
    parser.add_argument(
        "-m",
        "--matrix-type",
        dest="matrixType",
        default="channelnone",
        choices=list(filter_matrixType_poiPostfix.keys()),
        type=str,
        help="Select which matrix to read from file",
    )
    parser.add_argument(
        "--margin",
        default="0.2,0.16,0.07,0.2",
        type=str,
    ...
    parser.add_argument(
        ...
        default="0.2",
        type=str,
        help="Canvas bottom and left margins (same value, to make text fit)",
    )
    parser.add_argument(
        "--canvasSize",
        default="",
    ...
    if args.type == "toys":
        ...
        for l in lol:
            ...
            if "_err" in l.GetName():
                continue
            if "_minos" in l.GetName():
                continue
            if "_gen" in l.GetName():
                continue
            if "_In" in l.GetName():
                continue
    ...
    for ip1, p1 in enumerate(params):
        for ip2, p2 in enumerate(params):
            if args.type == "toys":
                var = "({x}-{x0})*({y}-{y0})".format(
                    x=p1, x0=fitvals[p1], y=p2, y0=fitvals[p2]
                )
    ...
    params = sorted(
        params, key=lambda x: 0 if "qGen1" in x else 1 if "qGen0" in x else 2
    )
    ...
    if not args.showAllNuisances:
        ...
        params = sorted(
            params,
            key=lambda x: (
                utilities.getNFromString(x, chooseIndex=0) if "pdf" in x else 0
            ),
        )
    ...
    c.SetGridy()
    if args.nContours:
        ROOT.gStyle.SetNumberContours(args.nContours)
    if args.palette:
        ROOT.gStyle.SetPalette(args.palette)
    ...
    nbins = len(params)
    th2_sub = ROOT.TH2D(
        "sub_corr_matrix", "correlation matrix", nbins, 0.0, nbins, nbins, 0.0, nbins
    )
    th2_cov = ROOT.TH2D(
        "sub_cov_matrix", "covariance matrix", nbins, 0.0, nbins, nbins, 0.0, nbins
    )
    ...
    th2_sub.GetXaxis().SetTickLength(0.0)
    th2_sub.GetYaxis().SetTickLength(0.0)
    th2_cov.GetXaxis().SetTickLength(0.0)
    th2_cov.GetYaxis().SetTickLength(0.0)
    ...
    for i, x in enumerate(params):
        for j, y in enumerate(params):
            if j > i:
                break
            sys.stdout.write(
                "Row {num}/{tot}   Column {col}/{tot}   \r".format(
                    num=i, tot=nParams, col=j
                )
            )
            sys.stdout.flush()
    ...
    th2_cov.GetZaxis().SetRangeUser(-1.0 * covMax, covMax)
    ...
    for im, tmp_mat in enumerate(matricesToPlot):
        ...
        if outdir:
            ...
            if len(params) < 30 and not args.noTextMatrix:
                tmp_mat.Draw("colz text45")
            else:
                tmp_mat.Draw("colz")
            ...
            if not args.skipLatexOnTop:
                ...
                lat.DrawLatex(
                    rightTextOffset + offsetLatex, 0.95, "16.8 fb^{-1} (13 TeV)"
                )

            if args.verticalLabelsX:
                tmp_mat.LabelsOption("v", "X")
            if nbins >= 20:
                tmp_mat.LabelsOption("v", "X")
            ...
            outfname = outdir + "/small{corcov}_{pn}{suff}".format(
                suff=suff, pn=paramsName, corcov=corcov
            )
            for i in ["pdf", "png"]:
