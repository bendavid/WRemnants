...
axis_pt_eff = hist.axis.Variable(axis_pt_eff_list, name = "pt", overflow=not args.excludeFlow, underflow=not args.excludeFlow, metadata="reco")
...
axis_eta = hist.axis.Regular(template_neta, template_mineta, template_maxeta, name = "eta", overflow=False, underflow=False, metadata="reco")
axis_pt = hist.axis.Regular(template_npt, template_minpt, template_maxpt, name = "pt", overflow=False, underflow=False, metadata="reco")

axis_charge = common.axis_charge
axis_passIso = common.axis_passIso
axis_passMT = common.axis_passMT
axis_passTrigger = hist.axis.Boolean(name = "passTrigger", metadata="reco")

nominal_axes = [axis_eta, axis_pt, axis_charge, axis_passIso, axis_passMT]

nominal_cols = ["goodMuons_eta0", "goodMuons_pt0", "goodMuons_charge0", "passIso", "passMT"]

axis_ut = hist.axis.Regular(40, -100, 100, overflow=True, underflow=True, name = "ut", metadata="reco")
...
axis_mt_fakes = hist.axis.Regular(120, 0., 120., name = "mt", underflow=False, overflow=True, metadata="reco")
axis_dphi_fakes = hist.axis.Regular(8, 0., np.pi, name = "DphiMuonMet", underflow=False, overflow=False, metadata="reco")
axis_hasjet_fakes = hist.axis.Boolean(name = "hasJets", metadata="reco") # only need case with 0 jets or > 0 for now
...
axis_eta_utilityHist = hist.axis.Regular(24, -2.4, 2.4, name = "eta", overflow=False, underflow=False, metadata="reco")
axis_pt_utilityHist = hist.axis.Regular(6, 26, 56, name = "pt", overflow=False, underflow=False, metadata="reco")

axis_met = hist.axis.Regular(100, 0., 200., name = "met", underflow=False, overflow=True, metadata="reco")
axis_recoWpt = hist.axis.Regular(40, 0., 80., name = "recoWpt", underflow=False, overflow=True, metadata="reco")
...
def build_graph(df, dataset):
    ...
    if dataset.is_data:
        ...
        results.append(nominal)
        results.append(df.HistoBoost("nominal_weight", [hist.axis.Regular(200, -4, 4, metadata="reco")], ["nominal_weight"], storage=hist.storage.Double()))
