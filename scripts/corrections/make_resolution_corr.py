...
sigmarelsq_data = sigmarelsq_data[:, 25.0j:]
sigmarelsq_mc = sigmarelsq_mc[:, 25.0j:]

smearingrel = sigmarelsq_data + -1.0 * sigmarelsq_mc
...
axis_eta_fine = hist.axis.Regular(
    neta_smooth, -2.4, 2.4, underflow=True, overflow=True, name="eta"
)
axis_pt_fine = hist.axis.Regular(
    npt_smooth, 25.0, 85.0, underflow=True, overflow=True, name="pt"
)
...
centers_fine_flat = [
    np.reshape(center, [-1]) for center in smearingrel_smooth.axes.centers
]
...
def check_errs(sigmarelsq, cov_sigmarelsq):
    ...
    if not np.all(np.isclose(r, 1.0)):
        raise ValueError(
            "Inconsistent variances between histogram and covariance matrix."
        )
...
axis_var = hist.axis.Integer(
    0, neig, underflow=False, overflow=False, name="smearing_variation"
)
...
for ieig in range(neig):
    print("ieig", ieig)
    spline_var = scipy.interpolate.RegularGridInterpolator(
        centers_flat,
        smearing_variations.values()[..., ieig],
        method="cubic",
        bounds_error=False,
        fill_value=None,
    )
...
# set overflow in eta to boundary values (but leave overflow in pt as 0)
smearing_variations_smooth.values(flow=True)[0, 1:-1] = (
    smearing_variations_smooth.values()[0]
)
smearing_variations_smooth.values(flow=True)[-1, 1:-1] = (
    smearing_variations_smooth.values()[-1]
)
