...
def parse_args(argv: List[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--inject-bkg-f0", type=float, default=0.0,
                   ...
                   "component with a basis-matched kinematic adjustment — "
                   "muon_kin: common per-muon pt rescale (η/φ/ρ and hence the "
                   "conditioning exactly unchanged); event_level: inverse "
                   "Collins–Soper reconstruction at fixed (yll, ptll/mll, "
                   "φll, cosθ*, φ*) (all 7 conditioning components fixed — "
                   "ptll itself scales with the drawn mass; the per-muon "
                   "kinematics move; muons drifting past the η "
                   "acceptance are zero-weighted). Either way the pseudo-data "
                   "kinematics stay self-consistent (mll = mll(pt)). "
    ...
    p.add_argument(
        ...
        "dilepton vars (yll, ln(ptll/mll), cosPhill, sinPhill, cosθ*, sinφ*, "
        "cosφ*) — every component DIMENSIONLESS, hence invariant under the "
        "common-pt-dilation mass direction (the leak-free criterion; plain "
        "ln ptll is forbidden — it moves 1:1 with the mass and biases the fit "
        "at first order in θ). Residual e/M/smear conditioning shifts "
        "propagate via the per-node recompute. event_level "
