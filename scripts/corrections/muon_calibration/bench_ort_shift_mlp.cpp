// Single-core ONNX Runtime C++ benchmark for the shift-reweight-MLP ONNX
// produced by ``train_shift_reweight_mlp_export.py`` with --export-onnx.
//
...
//         bench_ort_shift_mlp.cpp \
//         -lonnxruntime -lpthread \
//         -o bench_ort_shift_mlp
...
int main(int argc, char** argv) try {
    ...
    Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "bench_shift_mlp");
    ...
    if (n_inputs != 3 || n_outputs < 1) {
        std::fprintf(stderr,
            "expected 3 inputs and >=1 output; got %zu / %zu\n",
            n_inputs, n_outputs);
        return 3;
    }
    ...
    const int64_t n_u = infer_dim1(2, 3);

    std::fprintf(stderr,
        "model: %s\ninputs: %s[N,%ld], %s[N,%ld], %s[N,%ld]  outputs: %zu\n",
        onnx_path,
        in_names[0].c_str(), (long)n_y,
        in_names[1].c_str(), (long)n_c,
        in_names[2].c_str(), (long)n_u,
        n_outputs);
    ...
    for (int64_t B : batch_sizes) {
        ...
        binding.BindInput(in_names[2].c_str(), u_tensor);
