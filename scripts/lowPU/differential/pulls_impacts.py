...
def impacts_mu(filename, fOut):
    ...
    for k,xLabel in enumerate(xLabels):
        ...
        mu = getattr(tree, "%s" % xLabel)
        idx = int(np.where(xLabels_unsorted == xLabel)[0][0])
        impacts_ = np.array([round(impact_hist.GetBinContent(idx+1, i+1)/mu*scale, rounded) for i,yLabel in enumerate(yLabels)])

        # add total impact
        impacts_ = np.append(impacts_, round(getattr(tree, "%s_err" % xLabel)/mu*scale, rounded))
...
def getPullsAndImpacts(filename):
    ...
    name = "nuisance_impact_mu"
    name = "nuisance_impact_pmaskedexpnorm"
    impact_hist = fIn.Get(name)
    xLabels = np.array([impact_hist.GetXaxis().GetBinLabel(i) for i in range(1, impact_hist.GetNbinsX()+1)]) # POI
    yLabels = np.array([impact_hist.GetYaxis().GetBinLabel(i) for i in range(1, impact_hist.GetNbinsY()+1)]) # nuisances
...
if __name__ == '__main__':
    ...
    norm = True
    fIn = "CombineStudies/lowPU_differential/lowPU_Zmumu_RawPFMET_differential_tot_output.root"
    
    baseDir = "/eos/user/j/jaeyserm/www/wmass/combine/lowPU_differential/"

    impacts_mu(fIn, f"{baseDir}/lowPU_Zmumu_impacts_mu.html")
    impacts_avg(fIn, f"{baseDir}/lowPU_Zmumu_impacts_avg.html")

    df = getPullsAndImpacts(fIn)
    plotPullsAndImpacts(df, f"{baseDir}/lowPU_Zmumu_pulls_impacts_sortImpact.html", "impact")
    plotPullsAndImpacts(df, f"{baseDir}/lowPU_Zmumu_pulls_impacts_sortConstraint.html", "constraint", ascending=False)
