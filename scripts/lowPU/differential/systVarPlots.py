...
def doPlot(recoBinIdx=1, syst="", ratio=1.06):
    ...
    h_nom = fIn.Get("reco_mll_{proc}_mumu".format(proc=proc)).ProjectionY("h_nom", recoBinIdx, recoBinIdx)
    ...
    hUp = fIn.Get("reco_mll_{proc}_{syst}Up_mumu".format(proc=proc, syst=syst)).ProjectionY("h_up", recoBinIdx, recoBinIdx)
    hDw = fIn.Get("reco_mll_{proc}_mumu_{syst}Down_mumu".format(proc=proc, syst=syst)).ProjectionY("h_up", recoBinIdx, recoBinIdx)
...
if __name__ == "__main__":
    ...
    card = "CombineStudies/lowPU_differential/lowPU_Zmumu_RawPFMET_differential_output.root"
    outDir = "/eos/user/j/jaeyserm/www/wmass/combine/lowPU_differential/systVarPlots/"
    
    genBins = ["Zmumu_genBin1_mu", "Zmumu_genBin2_mu"]
    
    fIn = ROOT.TFile(card)
    
    doPlot(flavor="mu", syst="recoil_target_para_0", ratio=1.03)
    quit()
