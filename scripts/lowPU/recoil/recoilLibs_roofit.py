...
def doFitMultiGauss(bhist, comp, fitCfg, label, outDir_, binning_qT, funcJs=None, doFit=True, xMin=-100, xMax=100, yMin=1e-6, yMax=1e4, propagate=True, bkgCfg={}, yRatio = 1.15, recoilLow=-100, recoilHigh=100):
    ...
    for iBin in range(1, len(binning_qT)):
        ...
        pdfs_arglist.setName("pdfs_arglist_bin%d" % (iBin))
        norms_arglist.setName("norms_arglist_bin%d" % (iBin))
        bkg_ptrs = []
        for bkg in bkgCfg:
            if bkgCfg[bkg]['subtract']:
                ...
                pdfs_bkgs_arglist.setName("pdfs_bkgs_arglist_bin%d_%s" % (iBin, bkg))
                norms_bkgs_arglist.setName("norms_bkgs_arglist_bin%d_%s" % (iBin, bkg))
                for iGauss in range(1, jsIn['nGauss']+1):
                    mean = ROOT.RooRealVar("mean%d_bin%d_%s" % (iGauss, iBin, bkg), "", jsIn[str(iBin)]["mean%d" % iGauss], __min_mean__, __max_mean__)
                    sigma = ROOT.RooRealVar("sigma%d_bin%d_%s" % (iGauss, iBin, bkg), "", jsIn[str(iBin)]["sigma%d" % iGauss], __min_sigma__, __max_sigma__)
                    mean.setConstant(ROOT.kTRUE)
                    sigma.setConstant(ROOT.kTRUE)
                    gauss = ROOT.RooGaussian("gauss%d_bin%d_%s" % (iGauss, iBin, bkg), "", recoil, mean, sigma)
                ...
                for iGauss in range(1, jsIn['nGauss']):
                    norm = ROOT.RooRealVar("norm%d_bin%d_%s" % (iGauss, iBin, bkg), "", jsIn[str(iBin)]["norm%d" % iGauss], 0, 1)
                    norm.setConstant(ROOT.kTRUE)
                    norms_bkgs_arglist.addOwned(norm)
                    bkg_ptrs.append(norm)
                pdf_bkg = ROOT.RooAddPdf("bin%d_%s" % (iBin, bkg), '', pdfs_bkgs_arglist, norms_bkgs_arglist)
                pdfs_arglist.addOwned(pdf_bkg)
                norm_bkg = ROOT.RooRealVar("relyield_bin%d_%s" % (iBin, bkg), "", (jsIn[str(iBin)]['yield']*bkgCfg[bkg]['norm'])/yield_, 0, 1)
        ...
        pdf_tot = ROOT.RooAddPdf("pdf_tot_bin%d"%iBin, '', pdfs_arglist, norms_arglist)
        #pdfs_sig.append(pdf_tot)
        
        rdh = ROOT.RooDataHist("rdh_bin%d"%iBin, "", ROOT.RooArgList(recoil), ROOT.RooFit.Import(hist_root, ROOT.kFALSE))
        if dof > 0 and yield_ > 0:
        ...
        plt = recoil.frame()
        rdh.plotOn(plt) # defines the normalization
        if dof > 0 and yield_ > 0:
        ...
        latex.SetTextSize(0.035)
        for i, param in enumerate(mean_ptrs): latex.DrawLatex(0.7, 0.87-i*0.04, "#mu_{%d} = %.3f #pm %.3f" % (i+1, param.getVal(), param.getError() if fitCfg['mean_cfg'][i] >= 0 else mean_ptrs[abs(fitCfg['mean_cfg'][i])-1].getError())) 
        for i, param in enumerate(sigma_ptrs): latex.DrawLatex(0.7, 0.87-i*0.04-0.04*len(mean_ptrs), "#sigma_{%d} = %.3f #pm %.3f" % (i+1, param.getVal(), param.getError()))
        for i, param in enumerate(norm_ptrs): latex.DrawLatex(0.7, 0.87-i*0.04-0.04*len(mean_ptrs)*2, "n_{%d} = %.3f #pm %.3f" % (i+1, param.getVal(), param.getError()))
        ...
        histRatio = ratioHist(hist_root, histRatio, pdf_tot, recoil, norm)
        histRatio.Draw("SAME E0")
        ...
        # save output
        out[iBin] = {}
        out[iBin]['yield'] = yield_
        out[iBin]['yield_err'] = yield_err
        
        
        for i, param in enumerate(mean_ptrs):
            out[iBin]['mean%d' % (i+1)], out[iBin]['mean%d_err' % (i+1)] = param.getVal(), param.getError() if fitCfg['mean_cfg'][i] >= 0 else mean_ptrs[abs(fitCfg['mean_cfg'][i])-1].getError()
        
        for i, param in enumerate(sigma_ptrs):
            out[iBin]['sigma%d' % (i+1)], out[iBin]['sigma%d_err' % (i+1)] = param.getVal(), param.getError()
        
        for i, param in enumerate(norm_ptrs):
            out[iBin]['norm%d' % (i+1)], out[iBin]['norm%d_err' % (i+1)] = param.getVal(), param.getError()
    ...
    with open("%s/results.json" % outDir_) as f: out = json.load(f)
    doGlobalPlot(bhist, comp, out, binning_qT, outDir_, label, funcJs=funcJs, bkgCfg=bkgCfg, xMin=xMin, xMax=xMax, yMin=yMin, yMax=yMax, yRatio=yRatio, recoilLow=recoilLow, recoilHigh=recoilHigh)
...
def doGlobalPlot(bhist, comp, jsIn, binning_qT, outDir_, label, funcJs=None, bkgCfg={}, xMin=-100, xMax=100, yMin=1e-6, yMax=1e4, yRatio = 1.3, recoilLow=-100, recoilHigh=100):
    ...
    for iBin in range(1, len(binning_qT)):
        ...
        h =  h[{"qTbinned": s[0:hist.overflow:hist.sum]}] # remove the overflow in the sum
        hist_root = narf.hist_to_root(h)
        if totHist == None: totHist = hist_root.Clone("totHist")
        else: totHist.Add(hist_root)
        
        yield_, yield_err = h.sum().value, math.sqrt(h.sum().variance)
    

        pdfs, norms = ROOT.RooArgList(), ROOT.RooArgList()
        pdfs.setName("pdfs_bin%d" % iBin)
        norms.setName("norms_bin%d" % iBin)
        for iGauss in range(1, nGauss+1):
            mean_ = ROOT.RooRealVar("mean%d_bin%d" % (iGauss, iBin), "", jsIn[str(iBin)]['mean%d' % iGauss], __min_mean__, __max_mean__)
            sigma_ = ROOT.RooRealVar("sigma%d_bin%d" % (iGauss, iBin), "", jsIn[str(iBin)]['sigma%d' % iGauss], __min_sigma__, __max_sigma__)
            gauss = ROOT.RooGaussian("gauss%d_bin%d" % (iGauss, iBin), "", recoil, mean_, sigma_)
            pdfs.addOwned(gauss)
        ...
        for iGauss in range(1, nGauss):
            norm_ = ROOT.RooRealVar("norm%d_bin%d" % (iGauss, iBin), "", jsIn[str(iBin)]['norm%d' % iGauss], 0, 1)
            norms.addOwned(norm_)
            garbage.append(norm_)
        
        garbage.append(pdfs)
        garbage.append(norms)
        pdf_sig = ROOT.RooAddPdf("pdf_sig_bin%d" % (iBin), '', pdfs, norms)
        ptrs[pdf_sig.GetName()] = pdf_sig
        
        # include backgrounds (if configured)
        pdfs_arglist_, norms_arglist_ = ROOT.RooArgList(), ROOT.RooArgList()
        pdfs_arglist_.setName("pdfs_arglist_bin%d" % (iBin))
        norms_arglist_.setName("norms_arglist_bin%d" % (iBin))
        for bkg in bkgCfg:
        ...
        pdf_tot_ = ROOT.RooAddPdf("pdf_tot_bin%d" % (iBin), '', pdfs_arglist_, norms_arglist_)
        garbage.append(pdf_tot_)
        ptrs[pdf_tot_.GetName()] = pdf_tot_
        
        norm_bkg = ROOT.RooRealVar("totyield_bin%d" % (iBin), "", jsIn[str(iBin)]['yield']/totYield)
