...
def doFitMultiGauss(fInName, proc, comp, fitCfg, label, outDir_, recoil_qTbins, ratio=True, funcJs=None, excludeBins=[], qTmin=0, qTmax=300, doFit=True, rebin=1, xMin=-100, xMax=100, yMin=1e-6, yMax=1e4, propagate=True, bkgCfg={}, orderSigmas=False):
    ...
    for iGauss in range(1, nGauss+1):
        if fitCfg['mean_cfg'][iGauss-1] >= 0:
            mean = ROOT.RooRealVar("mean%d" % iGauss, "", fitCfg['mean'][iGauss-1], -10, 50)
        ...
        sigma = ROOT.RooRealVar("sigma%d" % iGauss, "", fitCfg['sigma'][iGauss-1], 1, 200)
    ...
    for qTbin in range(1, len(recoil_qTbins)):
        ...
        for bkg in bkgCfg:
            if bkgCfg[bkg]['subtract']:
                ...
                for iGauss in range(1, jsIn['nGauss']+1):
                    mean = ROOT.RooRealVar("mean%d_bin%d_%s" % (iGauss, qTbin, bkg), "", jsIn[str(qTbin)]["mean%d" % iGauss], -300, 300)
        ...
        if ratio:
            ...
            for i in range(1, histRatio.GetNbinsX()+1):
                ...
                pdf_eval = pdf_tot.getVal(norm)
                y, y_err = 0, 0
                if(pdf_eval > 0): 
                    y = histRatio.GetBinContent(i)/pdf_eval
                    y_err = histRatio.GetBinError(i)/pdf_eval
                histRatio.SetBinContent(i, y)
                histRatio.SetBinError(i, y_err)
                #if y > 0: print(i, histRatio.GetBinCenter(i), pdf_eval, y, y_err)
            histRatio.Draw("SAME E")
        else:
            plt = recoil.frame()
    ...
    doGlobalPlot(fInName, proc, comp, out, recoil_qTbins, outDir_, label, funcJs=funcJs, ratio=ratio, rebin=rebin, bkgCfg=bkgCfg, qTmin=qTmin, qTmax=qTmax, xMin=xMin, xMax=xMax, yMin=yMin, yMax=yMax)


def doGlobalPlot(fInName, proc, comp, jsIn, recoil_qTbins, outDir_, label, funcJs=None, ratio=True, qTmin=0, qTmax=50, rebin=1, bkgCfg={}, xMin=-100, xMax=100, yMin=1e-6, yMax=1e4):
    ...
    for qTbin in range(1, len(recoil_qTbins)):
        ...
        for iGauss in range(1, nGauss+1):
            mean_ = ROOT.RooRealVar("mean%d_bin%d" % (iGauss, qTbin), "", jsIn[str(qTbin)]['mean%d' % iGauss], -100, 100)
            sigma_ = ROOT.RooRealVar("sigma%d_bin%d" % (iGauss, qTbin), "", jsIn[str(qTbin)]['sigma%d' % iGauss], 0.1, 100)
    ...
    totHist.Scale(1./totHist.Integral())
    ...
    if doUnc:
        ...
        for nStat in range(0, nStatVars):
            ...
            for iGauss in range(1, nGauss+1):
                func_mean = ROOT.TF1("mean%d_%s" % (iGauss, nStatName), funcJs['mean%d' % iGauss]['func'], 0, 300)
                ...
                func_sigma = ROOT.TF1("sigma%d_%s" % (iGauss, nStatName), funcJs['sigma%d' % iGauss]['func'], 0, 300)
            ...
            for iGauss in range(1, nGauss):
                func_norm = ROOT.TF1("norm%d_%s" % (iGauss, nStatName), funcJs['norm%d' % iGauss]['func'], 0, 300)
            ...
            for qTbin in range(1, len(recoil_qTbins)):
                ...
                for iGauss in range(1, nGauss+1):
                    mean_ = ROOT.RooRealVar("mean%d_bin%d_%s" % (iGauss, qTbin, nStatName), "", funcs_mean[iGauss-1].Eval(qT), -100, 100)
                    sigma_ = ROOT.RooRealVar("sigma%d_bin%d_%s" % (iGauss, qTbin, nStatName), "", funcs_sigma[iGauss-1].Eval(qT), 0.1, 100)
                ...
                for bkg in bkgCfg:
                    ...
                    for iGauss in range(1, jsIn_bkg['nGauss']+1):
                        mean = ROOT.RooRealVar("mean%d_bin%d_%s_%s" % (iGauss, qTbin, bkg, nStatName), "", jsIn_bkg[str(qTbin)]["mean%d" % iGauss], -100, 100)
                        sigma = ROOT.RooRealVar("sigma%d_bin%d_%s_%s" % (iGauss, qTbin, bkg, nStatName), "", jsIn_bkg[str(qTbin)]["sigma%d" % iGauss], 0.1, 100)
                        mean.setConstant(ROOT.kTRUE)
                        sigma.setConstant(ROOT.kTRUE)
...
def prepareFile(datagroups, fOutName, fOut_):
...
def doFit(jsIn, jsOut, comp, param, fitF, iParams, outDir, recoil_qTbins, label, cParams=[], excludeBins=[], fitMin=0, fitMax=200, xMin=0, xMax=150, yMin=0, yMax=30, yTitle="", doFit=True, cutOffMin=-99999, cutOffMax=99999):
...
def combinedFit(fInName, proc, comp, fitCfg, jsIn, recoil_qTbins, qTmin=0, qTmax=50, rebin=1, singleMean=False, bkgCfg={}):
    ...
    for qTbin in range(1, len(recoil_qTbins)):
        ...
        for bkg in bkgCfg:
            ...
            for iGauss in range(1, jsIn_bkg['nGauss']+1):
                mean = ROOT.RooRealVar("mean%d_bin%d_%s" % (iGauss, qTbin, bkg), "", jsIn_bkg[str(qTbin)]["mean%d" % iGauss], -100, 100)
                sigma = ROOT.RooRealVar("sigma%d_bin%d_%s" % (iGauss, qTbin, bkg), "", jsIn_bkg[str(qTbin)]["sigma%d" % iGauss], 0.1, 200)
                mean.setConstant(ROOT.kTRUE)
                sigma.setConstant(ROOT.kTRUE)
                gauss = ROOT.RooGaussian("gauss%d_bin%d_%s" % (iGauss, qTbin, bkg), "", recoil, mean, sigma)
                garbage.append(mean)
