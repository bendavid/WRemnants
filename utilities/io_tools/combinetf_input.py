...
def get_poi_names(fitresult_file, poi_type="mu"):
    ...
    if len(names) == 0:
        logger.warning(
            "No free parameters found (neither signal strenght(s), nor W mass)"
        )
        return [None]
    return names
...
def get_poi_names_root(rtfile, poi_type="mu"):
    names = []
    if poi_type is not None and f"nuisance_impact_{poi_type}" in [
        k.replace(";1", "") for k in rtfile.keys()
    ]:
...
def read_impacts_poi(
    fileobject, group, poi, sort=True, add_total=True, stat=0.0, normalize=True
):
...
def read_impacts_poi_h5(h5file, group, poi, skip_systNoConstraint=False):
    if poi is None:
        ...
        if poi in poi_names:
            ...
            if poi_type in ["sumpois", "sumpoisnorm", "ratiometapois"]:
                ...
                norm_hist = "x"
            impact_hist = (
                f"nuisance_group_impact_{poi_type}"
                if group
                else f"nuisance_impact_{poi_type}"
            )
        else:
    ...
    if skip_systNoConstraint:
        noConstraint = np.array(
            [l in h5file["hsystsnoconstraint"][...].astype(str) for l in labels]
        )
...
def read_impacts_poi_root(rtfile, group, poi):
    ...
    if impact_hist not in rtfile:
        ...
        return np.zeros_like(labels), labels, 1.0, 1.0
    else:
...
def read_impacts_pois_h5(h5file, poi_type, group=True, uncertainties=None):
    ...
    impact_hist = (
        f"nuisance_group_impact_{poi_type}" if group else f"nuisance_impact_{poi_type}"
    )
...
def read_impacts_pois_root(rtfile, poi_type, group=True, uncertainties=None):
    ...
    if uncertainties is None:
        uncertainties = {
            f"err_{k}": impacts.values()[:, i] for i, k in enumerate(impacts.axes[1])
        }
    else:
        uncertainties = {
            f"err_{k}": impacts.values()[:, i]
            for i, k in enumerate(impacts.axes[1])
            if k in uncertainties
        }
...
def select_covariance_pois(
    cov, names, gen_axes=[], selections={}, base_processes=[], flow=False
):
    indices = filter_poi_bins(
        names, gen_axes, selections=selections, base_processes=base_processes, flow=flow
    )
...
def get_theoryfit_data(
    fitresult, axes, base_processes=["W"], poi_type="pmaskedexp", flow=False
):
    logger.info(
    ...
    cov = select_covariance_pois(
        cov, names, gen_axes=all_axes, base_processes=base_processes, flow=flow
    )
    ...
    for a, p in zip(axes, base_processes):
        ...
        hist_xsec.view(flow=False)[...] = np.stack(
            [df_c["value"].values, (df_c["err_total"].values) ** 2], axis=-1
        )
...
def read_groupunc_df(filename, uncs, rename_cols={}, name=None):
    ...
    df.iloc[0, 1] += (
        ref_massz if df.loc[0, "Name"] == "massShiftZ100MeV_noi" else ref_massw
    )
