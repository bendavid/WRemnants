...
def processDict():
    procs = OrderedDict(
        ...
        Wtaunu_minus={"label": "_nolegend_", "color": "darkblue"},
        Ztautau={
            "label": r"Z$\to\tau\tau$",
            "color": "orange",
        },
...
def processDictLowPileup(
    signalPtSplit=[0, 9, 20, 37, 60, 89, r"$\infty$"], removeUnsplit=False
):
    redshades = [
        "#ffcccc",
        "#ff9999",
    ...
    for i, ptlow in enumerate(signalPtSplit[:-1]):
        ...
        for proc in [
            ...
            processes[name] = dict(
                label=processes[proc]["label"]
                + r" (p$_{T}^{\mathrm{V}} \in$ "
                + f"[{ptlow}, {signalPtSplit[i+1]}])",
...
def fillLowPileupProcDictFromRoot(
    procDict,
    rtfile,
    syst,
    label="",
    obs="mt_reco_pf",
    axis_names=["qTreco", "iso", "charge", "mt"],
    makeVariable={},
    processes=[],
):
    if label == "":
...
def fillProcDictFromRoot(
    ...
    for k, v in procDict.items():
        ...
        if not rhist:
            if shouldRead:
                logging.warning(
                    f"No match for histogram '{syst}__{proc}' found in file"
                )
            v.update({label: None})
            continue
        axes = axis_names[:]
        if rhist.GetNdimensions() == len(axes) + 1:
            axes.append("systIdx")
        try:
            v.update({label: narf.root_to_hist(rhist, axis_names=axes)})
