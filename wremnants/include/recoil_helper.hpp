...
public:
  ...
  METXYCorrectionHelper(const c_t &coeff_x_, const c_t &coeff_y_,
                        const int &npv_min_, const int &npv_max_) {
    coeff_x = coeff_x_;
    coeff_y = coeff_y_;
    npv_min = npv_min_;
    npv_max = npv_max_;
  ...
  Vec_d operator()(const double &met_pt, const double &met_phi,
                   const int &npv) {
    ...
    // x
    delta = 0;
    for (int k = 0; k < coeff_x.size(); k++)
      delta += coeff_x.at(k) * std::pow(npv, k);
    if (std::abs(delta) > deltaMax)
      delta = 0;
    pUx -= delta;

    // y
    delta = 0;
    for (int k = 0; k < coeff_y.size(); k++)
      delta += coeff_y.at(k) * std::pow(npv, k);
    if (std::abs(delta) > deltaMax)
      delta = 0;
    pUy -= delta;
...
public:
  VPTReweightHelper(const std::vector<double> &bins_,
                    const std::vector<double> &weights_, const double pt_min_,
...
public:
  ResponseCorrector(const std::string &filename, const std::string &func_name)
      : helper_(std::make_shared<narf::tflite_helper>(
            filename, func_name, ROOT::GetThreadPoolSize())) {}
...
template <size_t N_UNC> class RecoilCalibrationHelper {
...
public:
  ...
  RecoilCalibrationHelper(const std::string &filename,
                          const std::string &func_name, bool do_unc_)
      : helper_(std::make_shared<narf::tflite_helper>(
            filename, func_name, ROOT::GetThreadPoolSize())),
        helper_no_unc_(std::make_shared<narf::tflite_helper>(
            filename, func_name + "_no_unc", ROOT::GetThreadPoolSize())) {
    do_unc = do_unc_;
  }

  out_t operator()(const double &pt, const double &ut_para,
                   const double &ut_perp) {
    ...
    if (do_unc) {
      auto outputs =
          std::tie(ret.ut_para_corr, ret.ut_perp_corr, ret.unc_weights);
      (*helper_)(inputs, outputs);
    } else {
    ...
    for (int k = 0; k < N_UNC; k++) {
      ...
      if (std::isnan(ret.unc_weights[k]) or std::isinf(ret.unc_weights[k])) {
        std::cout << k << " INF/NAN " << std::isnan(ret.unc_weights[k]) << " "
                  << std::isinf(ret.unc_weights[k]) << "  " << pt << " "
                  << ut_para << " " << ut_perp << std::endl;
      }
    }
    return ret;
  }
...
public:
  RecoilCalibrationUncertaintyHelper(const std::string &filename,
