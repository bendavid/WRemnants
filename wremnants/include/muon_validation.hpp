...
public:
  ...
  // qop the massWeights procedure doesn't work with multiple muons
  out_tensor_t operator()(
      float recoEta, int recoCharge, float recoPt,
      Eigen::TensorFixedSize<double, Eigen::Sizes<N_MASSWEIGHTS>> &weights,
      double nominal_weight = 1.0, bool isW = true) {
    ...
    for (std::ptrdiff_t ivar = 0; ivar < nUnc; ++ivar) {
      ...
      double scale = (abs(recoKUnc / recoK) > scaleCut
                          ? (std::copysign(1., recoKUnc / recoK) * scaleCut)
                          : (recoKUnc / recoK));
