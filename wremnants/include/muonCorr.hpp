...
double scaleWeight(double weight, double scale) {
  ...
  return std::exp(scale * std::log(std::abs(weight))) *
         std::copysign(1., weight);
}

template <size_t ETABINS, size_t T>
Eigen::TensorFixedSize<double, Eigen::Sizes<2, ETABINS>>
dummyScaleFromMassWeights(
    double nominal_weight,
    Eigen::TensorFixedSize<double, Eigen::Sizes<T>> &weights, double eta,
    double scale, bool isW = true) {
  ...
  if (centralIdx - step10MeV < 0) {
    throw std::out_of_range(
        "Maximum allowed range for momentum scale uncertainty is 100 MeV!");
  }
  ...
  outWeights(0, ieta) =
      scaleWeight(weights[centralIdx - step10MeV], scaleFac) * nominal_weight;
  outWeights(1, ieta) =
      scaleWeight(weights[centralIdx + step10MeV], scaleFac) * nominal_weight;
  return outWeights;
}
...
scaleShiftWeightsFromMassWeights(
    double nominal_weight,
    Eigen::TensorFixedSize<double, Eigen::Sizes<N_MASSWEIGHTS>> &weights,
    double scale, bool isW = true) {
  Eigen::TensorFixedSize<double, Eigen::Sizes<2>> res;
  // Down weight, then up weight
  res(0) = dummyScaleFromMassWeights<1, N_MASSWEIGHTS>(nominal_weight, weights,
                                                       0, scale, isW)(0, 0);
  res(1) = dummyScaleFromMassWeights<1, N_MASSWEIGHTS>(nominal_weight, weights,
                                                       0, scale, isW)(1, 0);
  return res;
}
