...
public:
  muon_efficiency_binned_helper_base(HIST_IDIPTRIGISO &&sf_idip_trig_iso,
                                     ...
                                     HIST_RECO &&sf_reco, std::string vqtstring)
    ...
    if ((!inputfile_->IsOpen()) || (inputfile_->IsZombie()))
      std::cout << "couldn't open " << vqtstring.c_str() << "\n";
    inputfile_->ls();
  ...
  muon_efficiency_binned_helper_base(HIST_IDIPTRIGISO &&sf_idip_trig_iso,
                                     HIST_TRACKING &&sf_tracking,
                                     HIST_RECO &&sf_reco, std::string vqtstring,
                                     std::string vqtplus, std::string vqtminus)
      : sf_idip_trig_iso_(std::make_shared<const HIST_IDIPTRIGISO>(
            std::move(sf_idip_trig_iso))),
        sf_tracking_(
            std::make_shared<const HIST_TRACKING>(std::move(sf_tracking))),
    ...
    if ((!inputfile_->IsOpen()) || (inputfile_->IsZombie()))
      std::cout << "couldn't open " << vqtminus.c_str() << "\n";
    ...
    if ((!inputfile_->IsOpen()) || (inputfile_->IsZombie()))
      std::cout << "couldn't open " << vqtplus.c_str() << "\n";
    trigger_.push_back((TH2F *)inputfile_->Get("SF2D_nominal"));
  ...
  double scale_factor_product(float pt, float eta, float sapt, float saeta,
    ...
    if (includeTrigger_) {
      ...
      sf *= trigger_[chargeidx]->GetBinContent(
          trigger_[chargeidx]->FindBin(eta, pt));
    }
  ...
  syst_tensor_t sf_syst_var(float pt, float eta, float sapt, float saeta,
    ...
    if (isohisto_->GetBinContent(isohisto_->FindBin(eta, pt)) > 0.)
      res(allSF_nomi.size() - 1) =
          (1. + isohisto_->GetBinError(isohisto_->FindBin(eta, pt)) /
                    isohisto_->GetBinContent(isohisto_->FindBin(eta, pt)));
    else
      res(allSF_nomi.size() - 1) = 1.;
    unsigned int chargeidx = (1 + charge) / 2;
    if (includeTrigger_) {
      if (trigger_[chargeidx]->GetBinContent(
              trigger_[chargeidx]->FindBin(eta, pt)) > 0)
        res(allSF_nomi.size() - 2) =
            (1. + trigger_[chargeidx]->GetBinError(
                      trigger_[chargeidx]->FindBin(eta, pt)) /
                      trigger_[chargeidx]->GetBinContent(
                          trigger_[chargeidx]->FindBin(eta, pt)));
      else
        res(allSF_nomi.size() - 2) = 1.;
    }
    return res;
  }
