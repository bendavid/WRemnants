...
def syst_transform_map(base_hist, hist_name):
    ...
    transforms["muonScaleUp"] = {"action" : lambda h: h if "unc" not in h.axes.name else hh.rssHistsAvg(h, "unc")[1]}
    transforms["muonScaleDown"] = {"action" : lambda h: h if "unc" not in h.axes.name else hh.rssHistsAvg(h, "unc")[0]}
    transforms["muonScale9Up"] = {"action" : lambda h: h if "unc" not in h.axes.name else hh.rssHistsAvg(h, "unc", 9)[1]}
    transforms["muonScale9Down"] = {"action" : lambda h: h if "unc" not in h.axes.name else hh.rssHistsAvg(h, "unc", 9)[0]}
