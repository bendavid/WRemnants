...
## TODO: use consistent names for filt and excl, filt was implemented before, should call it filterGroup
def getDatasets(maxFiles=-1, filt=None, excl=None, mode=None, base_path=None, nanoVersion="v9", 
    ...
    if excl:
        if isinstance(excl, list):
            ...
            narf_datasets = list(filter(lambda x: x.group not in excl if x.group is not None else not any(f in x.name for f in excl), narf_datasets))
        else:
            narf_datasets = list(filter(excl, narf_datasets))
