...
def cloneAxis(ax, overflow=False, underflow=False, newName=None, metadata=None):
    axName = newName if newName else ax.name
    if isinstance(ax, bh.axis.Regular):
        newax = hist.axis.Regular(ax.size, ax.edges[0], ax.edges[-1], name=axName, overflow=overflow, underflow=underflow, metadata=metadata)
    elif isinstance(ax, bh.axis.Variable):
        newax = hist.axis.Variable(ax.edges, name=axName, overflow=overflow, underflow=underflow, metadata=metadata)
    else:
        ...
        for charge, charge_tag in charges.items():
            for eff_type in axis_eff_type_3D:
                ...
                if sf_syst_3D is None:
                    ...
                    axis_eta_eff = cloneAxis(hist_hist.axes[0], overflow=True, underflow=True, metadata=hist_hist.axes[0])
                    axis_pt_eff = cloneAxis(hist_hist.axes[1], overflow=True, underflow=True, metadata=hist_hist.axes[1])
                    axis_ut_eff = cloneAxis(hist_hist.axes[2], overflow=True, underflow=True, metadata=hist_hist.axes[2])
