...
ROOT.gInterpreter.Declare('#include "muon_efficiencies_binned_vqt_real_old.h"')
...
def make_muon_efficiency_helpers_binned_vqt_real(filename = data_dir + "/testMuonSF/allSmooth_GtoH.root",
    ...
    for i in range(1,11):
        if i == 1 or i == 10:
            number = f"{i}newbinning"
        else:
            number = f"{i}"
        filename = f"{data_dir}/testMuonSF/3dsfseparately/{number}/mu_iso_both/allEfficiencies_2D.root"
        files.push_back(ROOT.std.string(filename))
    Vqtbins = ROOT.std.vector('float')()
    Vqtbins.push_back(-300.)
    Vqtbins.push_back(-30.)
    Vqtbins.push_back(-15.)
    Vqtbins.push_back(-10.)
    Vqtbins.push_back(-5.)
    Vqtbins.push_back(0.)
    Vqtbins.push_back(5.)
    Vqtbins.push_back(10.)
    ...
    Vqtbins.push_back(30.)
    Vqtbins.push_back(300.)
    ROOT.initializeDifferentialSFsNormal(files)

    if includeTrigger:
        filestriggerplus = ROOT.std.vector('std::string')()
        filestriggerminus = ROOT.std.vector('std::string')()
        for i in range(1,11):
            if i == 1 or i == 10:
                number = f"{i}newbinning"
            else:
                number = f"{i}"
            filename = f"{data_dir}/testMuonSF/3dsfseparately/{number}/mu_trigger_plus/allEfficiencies_2D.root"
            filestriggerplus.push_back(ROOT.std.string(filename))
            filename = f"{data_dir}/testMuonSF/3dsfseparately/{number}/mu_trigger_minus/allEfficiencies_2D.root"
            filestriggerminus.push_back(ROOT.std.string(filename))
        ROOT.initializeDifferentialSFsNormalTrigger(filestriggerplus,filestriggerminus)

    ROOT.initializeVQTBinning(Vqtbins)
