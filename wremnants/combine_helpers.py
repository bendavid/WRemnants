...
def varyHistByBin(hnom, 
    ...
    **index_info
    ...
    # move given axes to the left 
    moves = [i for i, n in enumerate(hnom.axes.name) if n in index_info.keys()]

    moved_values = np.moveaxis(values, moves, np.arange(len(moves)))

    # set variation
    indices = list(index_info.values())
    indices = tuple(indices + [slice(None)] * (len(hnom.axes.name) - len(index_info.values())))
    moved_values[indices] = hnom[index_info].values(flow=True) * (1 + variation_magnitude)

    # move back to original order
    varied_values = np.moveaxis(moved_values, np.arange(len(moves)), moves)
    hvar.values(flow=True)[...] = varied_values

    return hvar

def setSignalBinsAsNuisances(cardTool,
    base_group,
    new_name=None,
    passToFakes=False,
    member_filter=None
):  
    # add a systematic for each gen bin where the contribution of the gen bin is varied

    # need to load base histogram to know how many gen bins are defined
    if base_group not in cardTool.datagroups.groups.keys():
        raise RuntimeError(f"Base group {base_group} not found in groups {cardTool.datagroups.groups.keys()}!")

    base_members = cardTool.datagroups.groups[base_group].members[:]
    if member_filter is not None:
        base_members = [m for m in filter(lambda x, f=member_filter: f(x), base_members)]            

    if cardTool.nominalName not in cardTool.datagroups.results[base_members[0].name]["output"].keys():
        raise RuntimeError(f"Nominal histogram '{cardTool.nominalName}' for member '{base_members[0].name}' not found.")

    nominal_hist = cardTool.datagroups.results[base_members[0].name]["output"][cardTool.nominalName].get()

    base_name = base_group if new_name is None else new_name

    # loop over all gen bins
    gen_bin_indices = cardTool.datagroups.getGenBinIndices(nominal_hist)
    for indices in itertools.product(*gen_bin_indices):

        # define variation
        index_info = {var : idx for idx, var in zip(indices, cardTool.datagroups.gen_axes)}
        bin_name = "_".join([f"{var}{idx}" for idx, var in zip(indices, cardTool.datagroups.gen_axes)])

        cardTool.addSystematic(f"r{base_name}_{bin_name}",
            rename="",
            processes=[base_group],
            group=f"r{base_name}_{bin_name}",
            noi=True,
            noConstraint=True,
