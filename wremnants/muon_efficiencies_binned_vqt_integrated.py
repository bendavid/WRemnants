...
def make_muon_efficiency_helpers_binned_vqt_integrated(filename = data_dir + "/muonSF/allSmooth_GtoH.root", filenamevqt = data_dir + "/muonSF/allSmooth_GtoH.root", filenamevqttriggerplus = "/gpfs/ddn/cms/user/bruschin/newfit3/egm_tnp_analysis/plots/triggerefficiencies17012023/efficiencies_GtoH/mu_trigger_plus/allEfficiencies_2D.root", filenamevqttriggerminus = "/gpfs/ddn/cms/user/bruschin/newfit3/egm_tnp_analysis/plots/triggerefficiencies17012023/efficiencies_GtoH/mu_trigger_minus/allEfficiencies_2D.root",
    ...
    axis_nom_syst = hist.axis.Integer(0, 2, underflow = False, overflow =False, name = "nom-syst", metadata={"type":"syst"}) # only one syst for now (and the nominal in the first bin)
            ...
            for eff_type in effSyst_manager[effSystKey]["axisLabels"]:
                for nomiAltId,nomiAltTag in nomiAltTypes.items():
                    ...
                    if effSyst_manager[effSystKey]["boostHist"] is None:
                        ...
                        axis_eff_type = hist.axis.StrCategory(effSyst_manager[effSystKey]["axisLabels"], name = f"{effSystKey}_eff_type", metadata={"type":"syst"})
    ...
    if includeTrigger:
        ...
        helper = ROOT.wrem_vqt_int.muon_efficiency_binned_helper[str(is_w_like).lower(),
    ...
    #                                                      ROOT.std.move(sf_reco_pyroot),
    #                                                  )
    helper_syst = ROOT.wrem_vqt_int.muon_efficiency_binned_helper_syst[str(is_w_like).lower(),
                                                                       type(sf_other_pyroot),
                                                                       type(sf_tracking_pyroot),
                                                                       type(sf_reco_pyroot)](
                                                                           helper
    ...
    for effStatKey in effStat_manager.keys():
        ...
        if isinstance(axis_pt_eff, bh.axis.Regular):
            axis_pt_eff_tensor = hist.axis.Regular(axis_pt_eff.size, axis_pt_eff.edges[0], axis_pt_eff.edges[-1], name = "nPtBins", overflow = False, underflow = False, metadata={"type":"syst"})
        elif isinstance(axis_pt_eff, bh.axis.Variable):
            axis_pt_eff_tensor = hist.axis.Variable(axis_pt_eff.edges[:nptbins+1], name = "nPtBins", overflow = False, underflow = False, metadata={"type":"syst"})
