...
def make_corr_helper_fromnp(filename=f"{common.data_dir}/N3LLCorrections/inclusive_{{process}}_pT.npz", isW=True):
    ...
    axis_syst = hist.axis.Regular(len(bins[0]) - 1, bins[0][0], bins[0][-1], 
                    name="systIdx", overflow=False, underflow=False, metadata="syst")
    axis_mass = hist.axis.Variable(bins[1], name="mass", metadata="gen")
    axis_y = hist.axis.Variable(bins[2], name="y", metadata="gen")
    axis_pt = hist.axis.Regular(len(bins[-1]) - 1, bins[-1][0], bins[-1][-1], name="pT", underflow=False, metadata="gen")
...
def make_corr_by_helicity(ref_helicity_hist, target_sigmaul, target_sigma4, coeff_hist=None, coeffs_from_hist=[], binning=None, ndim=3):
    ...
    corr_ax = hist.axis.Boolean(name="corr", metadata="syst")
    vars_ax = target_sigmaul.axes["vars"] if target_sigmaul else hist.axis.Regular(1 ,0, 1, name="vars", metadata="syst")
...
def make_qcd_uncertainty_helper_by_helicity(is_w_like = False, filename=None):
    ...
    vars_ax = hist.axis.StrCategory(var_names, name="vars", metadata="syst")
...
def make_helicity_test_corrector(is_w_like = False, filename = None):
    ...
    corr_ax = hist.axis.Boolean(name="corr", metadata="syst")
    vars_ax = hist.axis.StrCategory(["test_ai", "test_sigmaUL","test_all"], name="vars", metadata="syst")
...
def read_corr(generator, corr_files, charge, axes=[]):
    if "scetlib" in generator:
        ...
        vars_ax = h.axes["vars"] if "vars" in h.axes.name else hist.axis.StrCategory(["central"], name="vars", metadata="syst") 
